...
func scrape(url string) (lunchdata.Restaurants, error) {
	...
	log.Infof(
		"%s: Starting scrape of %q @ %s",
		logTag,
		url,
		t_start.Format(time.RFC3339),
	)
	...
	log.Infof("%s: Parsed %d restaurants with %d dishes in total", logTag, rs.Len(), rs.NumDishes())
...
func update() error {
	...
	// we need to set the GTAG for the results.
	// we shouldn't really do this here, as the scraper should not know about the tag,
	// but for now, until we have separated the scraper completely, we do it here
	rs.PropagateGtag(_gtag) // _gtag should be set via cli args
